...
本地 CLI 会话分别存储在 `journal.root_dir/sessions` 下，多个终端可在同一项目中工作。
同一会话同时只允许一个终端控制；浏览和恢复同时包含新会话与旧日志。
`serve` 保留共享 Host 的日志目录和写锁，浏览命令可以并行只读访问。
...
后端在报告用量或生成内容之前明确拒绝上下文容量时，使用独立的恢复策略。默认恢复
一次，以被拒绝输入规划量的一半作为优先压缩目标，保留任务、权限事实和完整交换边界。
必须保留的上下文无法容纳时，可以在仍小于被拒绝规划量的上限内重新尝试压缩，但不扩大
Run 累计预算。新请求占用一个模型步骤，已完成的工具不会重复执行。不可压缩内容仍超限
或再次被拒绝时明确结束。
OpenAI 兼容适配器要求 HTTP 400 且 `error.code = context_length_exceeded`；普通参数
错误和生成后的错误不触发恢复。拒绝事实和更小的输入预算会持久化，重启后仍有效。
成功生成只重置连续重试计数，更小的输入上限会保留到当前 Run 结束，约束后续工具轮次。
压缩分页工具结果时，摘要优先保留完整引用与下一页游标；需要原文时仍可回查 Session。
...
agent:
  ...
  model_retry:
    ...
    max_delay_ms: 8000
  context_recovery:
    max_retries: 1 # 0 禁用明确上下文容量拒绝后的恢复
```
